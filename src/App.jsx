...
import { FESTIVALS, REGIONS, MONTHS } from './data/festivals.js';
    ...
    <header className="site-header">
      <div className="logo">
        <svg viewBox="0 0 40 40" width="36" height="36" aria-hidden>
          ...
          <path d="M 20 6 L 20 12 M 20 28 L 20 34 M 6 20 L 12 20 M 28 20 L 34 20"
      ...
      <nav className="site-nav">
        <a href="#" className="nav-link nav-link--active">音樂祭</a>
        <a href="#" className="nav-link">月曆</a>
        <a href="#" className="nav-link">專欄</a>
        <a href="#" className="nav-link">收藏</a>
...
// ───────────────────────── Banner with map ─────────────────────────
function Banner({ activeMonth, setActiveMonth, festivals, activeRegion, setActiveRegion, onPinClick, hoveredId, setHoveredId }) {
    ...
    <section className="banner">
      <div className="banner-text">
        ...
        <div className="banner-stats">
          <div className="stat">
            <div className="stat-num serif">{String(monthCount).padStart(2, "0")}</div>
            <div className="stat-label mono">場 · {MONTHS[activeMonth - 1]}</div>
          </div>
          <div className="stat-divider" />
          <div className="stat">
            <div className="stat-num serif">{String(totalCount).padStart(2, "0")}</div>
    ...
    <aside className="filter-panel">
      <div className="panel-head">
        <div>
          ...
          <p className="panel-sub mono">FILTER · {String(results).padStart(2, "0")} 場結果</p>
        </div>
        <button className="link-reset mono" onClick={onReset}>清除</button>
      </div>
      ...
      <div className="search-box">
        <svg viewBox="0 0 24 24" width="16" height="16" className="search-icon">
          ...
          <line x1="15.5" y1="15.5" x2="20" y2="20" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
        </svg>
        <input
          type="text"
        ...
        {state.q &&
        <button className="search-clear" onClick={() => setState((s) => ({ ...s, q: "" }))}>×</button>
        }
      </div>
      ...
      <FilterSection
        ...
        <div className="chip-grid">
          ...
          <button
            ...
            onClick={() => setState((s) => ({ ...s, region: s.region === r ? null : r }))}>
      ...
      <FilterSection
        ...
        <div className="month-range-picker">
          {/* 提示文字：目前處於哪一步 */}
          <div className={`month-range-hint mono ${pickingEnd ? "month-range-hint--picking" : ""}`}>
            {pickingEnd
      ...
      <div className="panel-actions">
        <button className="btn-primary" onClick={onApply}>套用篩選</button>
        <button className="btn-secondary" onClick={onReset}>重設</button>
      </div>
    ...
    <article
      ...
      <div className="fest-card-body">
        ...
        <div className="fest-card-foot">
          ...
          <span className="fest-id mono">No. {f.id.slice(0, 6).toUpperCase()}</span>
        </div>
      </div>
    </article>);
...
function ResultsPanel({ festivals, sortMode, setSortMode, hoveredId, setHoveredId, onSave, activeRegion, monthStart, monthEnd }) {
  const [page, setPage] = useState(0);

  // 篩選條件改變時回到第一頁
  useEffect(() => { setPage(0); }, [festivals]);
  ...
  // 時間範圍顯示文字
  const timeLabel = (monthStart === 1 && monthEnd === 12)
    ? "全年"
    : monthStart === monthEnd
      ? `${monthStart} 月`
      ...
      <div className="results-head">
        ...
        <div className="results-tools">
          <div className="sort-group mono">
            <button className={`sort-btn ${sortMode === "date" ? "sort-btn--on" : ""}`} onClick={() => setSortMode("date")}>日期</button>
            <button className={`sort-btn ${sortMode === "region" ? "sort-btn--on" : ""}`} onClick={() => setSortMode("region")}>地區</button>
            <button className={`sort-btn ${sortMode === "saved" ? "sort-btn--on" : ""}`} onClick={() => setSortMode("saved")}>收藏</button>
          </div>
          <div className="view-group mono">
            <button className="view-btn view-btn--on" aria-label="grid">
              <svg viewBox="0 0 16 16" width="14" height="14"><rect x="1" y="1" width="6" height="6" fill="none" stroke="currentColor" strokeWidth="1" /><rect x="9" y="1" width="6" height="6" fill="none" stroke="currentColor" strokeWidth="1" /><rect x="1" y="9" width="6" height="6" fill="none" stroke="currentColor" strokeWidth="1" /><rect x="9" y="9" width="6" height="6" fill="none" stroke="currentColor" strokeWidth="1" /></svg>
            </button>
            <button className="view-btn" aria-label="list">
              <svg viewBox="0 0 16 16" width="14" height="14"><line x1="2" y1="4" x2="14" y2="4" stroke="currentColor" strokeWidth="1.2" /><line x1="2" y1="8" x2="14" y2="8" stroke="currentColor" strokeWidth="1.2" /><line x1="2" y1="12" x2="14" y2="12" stroke="currentColor" strokeWidth="1.2" /></svg>
            </button>
          </div>
        </div>
      </div>
...
function PalettePicker({ value, onChange }) {
  const opts = [
  { v: "warm", label: "Warm camel", colors: ["#FAF7F2", "#F1EBE0", "#B89B7A", "#7C8A4E", "#2A2622"] },
  { v: "moss", label: "Moss field", colors: ["#F4F2EB", "#E8E8DA", "#8B9670", "#A6543B", "#1F2A22"] },
  { v: "dusk", label: "Dusk plum", colors: ["#F2F0F1", "#E6E2E4", "#8C7A91", "#C49362", "#262128"] }];
...
function App() {
  ...
  const BLANK_FILTER = { q: "", region: null, genres: [], monthStart: 1, monthEnd: 12 };
  ...
  const visible = useMemo(() => {
    ...
    if (applied.genres.length) out = out.filter((f) => applied.genres.includes(f.genre));
    if (applied.q.trim()) {
    ...
    // 月份範圍篩選
    out = out.filter((f) => f.month >= applied.monthStart && f.month <= applied.monthEnd);
    if (sortMode === "date") out.sort((a, b) => a.dateStart.localeCompare(b.dateStart));
    if (sortMode === "region") out.sort((a, b) => a.region.localeCompare(b.region));
    if (sortMode === "saved") out.sort((a, b) => Number(b.saved) - Number(a.saved));
    return out;
  }, [festivals, applied, sortMode]);

  const handleSave = (id) => {
    setFestivals((fs) => fs.map((f) => f.id === id ? { ...f, saved: !f.saved } : f));
  };
